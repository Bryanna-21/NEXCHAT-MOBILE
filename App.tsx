...
import {Alert,Animated,FlatList,Image,Modal,SafeAreaView,ScrollView,Share,StatusBar,StyleSheet,Switch,Text,TextInput,TouchableOpacity,View} from "react-native";
...
function Button({label,onPress,secondary=false,danger=false}:{label:string;onPress:()=>void;secondary?:boolean;danger?:boolean}){return <TouchableOpacity onPress={onPress} style={[s.button,secondary&&s.secondary,danger&&s.danger]}><Text style={[s.buttonText,secondary&&{color:"#123"},danger&&{color:"white"}]}>{label}</Text></TouchableOpacity>}
...
function ScreenTransition({screenKey,children}:{screenKey:string;children:React.ReactNode}){const opacity=useRef(new Animated.Value(0)).current;const translateY=useRef(new Animated.Value(10)).current;useEffect(()=>{opacity.setValue(0);translateY.setValue(10);Animated.parallel([Animated.timing(opacity,{toValue:1,duration:180,useNativeDriver:true}),Animated.timing(translateY,{toValue:0,duration:180,useNativeDriver:true})]).start()},[screenKey]);return <Animated.View style={{flex:1,opacity,transform:[{translateY}]}}>{children}</Animated.View>}
