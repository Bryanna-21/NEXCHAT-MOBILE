...
/**
 * Record a short video with the device camera.
 *
 * Mirrors capturePhoto() but requests video capture mode. Used
 * by Stories (and could be reused anywhere video capture is
 * needed) — the recorded clip goes through the same durable
 * attachment ingestion pipeline as every other attachment.
 */
export async function captureVideo(): Promise<Attachment | null> {
  const permission =
    await ImagePicker.requestCameraPermissionsAsync();
